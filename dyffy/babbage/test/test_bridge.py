...
class TestBridge(unittest.TestCase):
    ...
    def test_walletunlock(self):
        """Bridge.walletunlock"""
        # self.assertRaises(self.bridge.sendfrom(self.user_id, self.address,
        #                                        self.amount_to_send),
        #                   urllib2.HTTPError)
    ...
    def test_sendfrom(self):
        """Bridge.sendfrom"""
        # start_balance = self.bridge.getbalance(self.user_id, as_decimal=True)
        self.bridge.walletunlock(config.COINS[self.bridge.coin]["passphrase"],
                                 config.COINS[self.bridge.coin]["unlock-timeout"])
        txid = self.bridge.sendfrom(self.user_id, self.address, self.amount_to_send)
        self.assertEqual(type(txid), str)
        # end_balance = self.bridge.getbalance(self.user_id, as_decimal=True)
        # self.assertEqual(start_balance - end_balance, Decimal(self.amount_to_send))

    def test_signmessage(self):
        """Bridge.signmessage"""
        # self.assertRaises(self.bridge.signmessage(self.btc_testnet_faucet,
        #                                           self.message),
        #                   urllib2.HTTPError)
